...
impl History {
    ...
    pub fn push(&mut self, ao: ActionObservation, bool_new_turn: bool) {
        ...
        if bool_new_turn {
            ...
            debug_assert!(
                self.current_player_turn == ao.player_id(),
                "Player turn not the same as ActionObservation"
            );
        } else {
    ...
    pub fn log_state(&self) {
        ...
        log::info!("Current_player_turn: {:?}", self.current_player_turn);
        log::info!("Influence: {:?}", self.latest_influence());
        log::info!("Coins: {:?}", self.latest_coins());
        // log::info!("{}", format!("Store_len: {:?}", self.store_len));
        log::info!("Dead Cards: {:?}", self.public_card_count);
    }
    ...
    pub fn log_history(&self) {
        log::info!(
            "Current_player_turn: {:?}",
            self.get_history(self.store_len)
        );
        println!(
            "Current_player_turn: {:?}",
            self.get_history(self.store_len)
    ...
    pub fn pop(&mut self) {
        ...
        if self.dist_from_turn[self.store_len - 1] == 0 {
            ...
            self.current_player_turn = self.store
                [self.store_len - 1 - self.dist_from_turn[self.store_len - 2]]
                .player_id();
        }
    ...
    pub fn subtract_public_card_count(&mut self, card: &Card) {
        debug_assert!(
            self.public_card_count[*card as usize] > 0,
            "Card already at zero! Cannot decrease further!"
        );
        // *self.public_card_count.entry(*card).or_insert(0) -= 1;
        self.public_card_count[*card as usize] -= 1;
    }
    ...
    pub fn get_dist_from_turn(&self, len: usize) -> Vec<usize> {
        debug_assert!(
            len <= MAX_HISTORY_LEN,
            "Use a proper len in get_dist_from_turn!"
        );
    ...
    pub fn push_ao(&mut self, ao: ActionObservation) {
        ...
        .contains(&ao_name)
        {
            ...
            if self.store_len >= 3 && self.store[self.store_len - 3].name() == AOName::Tax {
                let tax_id: usize = self.store
                    [self.store_len - self.dist_from_turn[self.store_len - 1]]
                    .player_id();
            ...
            // Case FA4 C3 C8 D3
            if self.store_len >= 4 {
                if self.store[self.store_len - 3].name() == AOName::BlockSteal
                    && self.store[self.store_len - self.dist_from_turn[self.store_len - 1]].name()
                        == AOName::Steal
                {
                    // Case C3 C8
                    // Coup Discard, can lookback to previous move -5 as BlockSteal so we include second condition
                    let stealer_id: usize = self.store
                        [self.store_len - self.dist_from_turn[self.store_len - 1]]
                        .player_id();
                    let stolen_id: usize = self.store
                        [self.store_len - self.dist_from_turn[self.store_len - 1]]
                        .opposing_player_id();
                    let amount: u8 = self.store
                        [self.store_len - self.dist_from_turn[self.store_len - 1]]
                        .amount();
                        ...
                        == AOName::ForeignAid
                {
                    ...
                    // if Blocker dead
                    // Case C6 C7 C8 C9
                    if self.latest_influence()[self.store
                        [self.store_len - self.dist_from_turn[self.store_len - 1]]
                        .opposing_player_id()]
                        == 0
                    {
                ...
                && self.store[self.store_len - 3].name() == AOName::BlockSteal
                && self.store[self.store_len - self.dist_from_turn[self.store_len - 1]].name()
                    == AOName::Steal
            {
                // Case C3 C8
                ...
                let stolen_id: usize = self.store
                    ...
                    .opposing_player_id();
                let amount: u8 =
                    self.store[self.store_len - self.dist_from_turn[self.store_len - 1]].amount();
                if self.latest_influence()[stealer_id] != 0 {
                    self.add_coins(stealer_id, amount);
                }
                if self.latest_influence()[stolen_id] != 0 {
        ...
        } else if ao.name() == AOName::CollectiveBlock
            && ao.final_actioner() == ao.opposing_player_id()
        {
    ...
    pub fn add_moves(
        ...
        match move_name {
            AOName::Income => {
                changed_vec.push(ActionObservation::Income { player_id });
            }
            AOName::ForeignAid => {
                changed_vec.push(ActionObservation::ForeignAid { player_id });
            }
            AOName::Tax => {
                changed_vec.push(ActionObservation::Tax { player_id });
            }
            AOName::Steal => {
                while id != player_id {
                    if self.latest_influence()[id] > 0 {
                        if self.latest_coins()[id] > 1 {
                            changed_vec.push(ActionObservation::Steal {
                                player_id,
                                opposing_player_id: id,
                                amount: 2,
                            });
                        } else {
                            changed_vec.push(ActionObservation::Steal {
            ...
            AOName::Assassinate => {
                if self.latest_coins()[player_id] >= 3 {
                    while id != player_id {
                        if self.latest_influence()[id] > 0 {
                            changed_vec.push(ActionObservation::Assassinate {
                                player_id,
                                opposing_player_id: id,
                            });
                        }
            ...
            AOName::Coup => {
                if self.latest_coins()[player_id] >= 7 {
                    while id != player_id {
                        if self.latest_influence()[id] > 0 {
                            changed_vec.push(ActionObservation::Coup {
                                player_id,
                                opposing_player_id: id,
                            });
                        }
            ...
            AOName::CollectiveChallenge => {
                ...
                changed_vec.push(ActionObservation::CollectiveChallenge {
                    ...
                    opposing_player_id,
                    final_actioner: opposing_player_id,
                });
                ...
                for comb in 1..(1 << 6) {
                    ...
                    if any_true {
                        for (i, participant_check) in participants.iter().enumerate() {
            ...
            AOName::BlockSteal => {
                let stealer_id: usize = self.store
                    [self.store_len - self.dist_from_turn[self.store_len - 1]]
                    .player_id();
                changed_vec.push(ActionObservation::BlockSteal {
                    player_id,
                    opposing_player_id: stealer_id,
            ...
            AOName::BlockAssassinate => {
                ...
                let attacker_id: usize = self.store
                    [self.store_len - self.dist_from_turn[self.store_len - 1]]
                    .player_id();
                changed_vec.push(ActionObservation::BlockAssassinate {
                    player_id,
                    opposing_player_id: attacker_id,
                });
            ...
            AOName::Discard => {
                ...
                if num_dead_amb < 3 {
                    changed_vec.push(ActionObservation::Discard {
                        player_id,
            ...
            AOName::RevealRedraw => {
                ...
                if self.store_len >= 2 {
                    if self.store[self.store_len - 2].name() == AOName::Assassinate {
                        ...
                        if num_dead_ass < 3 {
                            changed_vec.push(ActionObservation::RevealRedraw {
                                player_id,
                        ...
                        if self.latest_influence()[player_id] == 1 {
                            ...
                            debug_assert!(
                                self.latest_influence()[player_id] == 2,
                                "Improper Influence reached in add_moves discard"
                            );
                            if num_dead_amb < 2 {
                                changed_vec.push(ActionObservation::Discard {
                                    player_id,
                            ...
                            if num_dead_cpt < 3 && num_dead_duk < 3 {
                                changed_vec.push(ActionObservation::Discard {
                                    player_id,
                        ...
                        && self.store[self.store_len - 3].name() == AOName::ForeignAid
                    {
                        // FA3
                        if num_dead_duk < 3 {
                            changed_vec.push(ActionObservation::RevealRedraw {
            ...
            AOName::Exchange => {
                changed_vec.push(ActionObservation::Exchange { player_id });
            }
            AOName::ExchangeChoice => {
                ...
                if bool_know_priv_info {
                        ...
                        if total_counts[i] + count_card_arr[i] < 4 {
                            for j in i..5 {
                                ...
                                if total_counts[j] + count_card_arr[j] < 4 {
                                    changed_vec.push(ActionObservation::ExchangeChoice {
                                        player_id,
                                        relinquish: [
                    ...
                    //                                     changed_vec.push(ActionObservation::ExchangeChoice { player_id, no_cards: 2, hand: [Card::try_from(i_p as u8).unwrap(), Card::try_from(j_p as u8).unwrap()], relinquish: [Card::try_from(i as u8).unwrap(), Card::try_from(j as u8).unwrap()]});
                    //                                 }
                    //                                 total_counts[j] -= 1;
                    //                             }
                    //                         }
                    ...
                    //                         total_counts[j] += 1;
                    //                         if total_counts[j] + count_card_arr[j] < 3 {
                    //                             changed_vec.push(ActionObservation::ExchangeChoice { player_id, no_cards: 1, hand: [Card::try_from(i_p as u8).unwrap(), Card::try_from(i_p as u8).unwrap()], relinquish: [Card::try_from(i as u8).unwrap(), Card::try_from(j as u8).unwrap()]});
                    //                         }
                    //                         total_counts[j] -= 1;
                    //                     }
                    //                 }
                    ...
                    //         changed_vec.push(ActionObservation::ExchangeChoice { player_id, no_cards: self.latest_influence()[player_id], hand: [Card::Ambassador, Card::Ambassador], relinquish: [Card::try_from(i as u8).unwrap(), Card::try_from(i as u8).unwrap()]});
                    //     }
                    //     for j in (i + 1)..5 {
                    //         if count_card_arr[i] < 3 && count_card_arr[j] < 3 {
                    //             changed_vec.push(ActionObservation::ExchangeChoice { player_id, no_cards: self.latest_influence()[player_id], hand: [Card::Ambassador, Card::Ambassador], relinquish: [Card::try_from(i as u8).unwrap(), Card::try_from(j as u8).unwrap()]});
                    //         }
                    //     }
                    // }
            ...
            _ => panic!(
                "{}",
                format!("add_move for AOName {:?} not implemented", move_name)
            ),
        }
    }
    ...
    pub fn generate_legal_moves(&self, private_player: Option<usize>) -> Vec<ActionObservation> {
        ...
        match self.store[self.store_len - 1].name() {
            AOName::EmptyAO => {
                ...
                debug_assert!(
                    false,
                    "Generating Legal Moves for EmptyAO not at the start of the game!"
                );
            }
            AOName::Income => {
                // Case Checked
            ...
            AOName::Tax | AOName::Steal | AOName::Exchange | AOName::Assassinate => {
                // Case Checked
                let last_player: usize = self.store[self.store_len - 1].player_id();
                self.add_moves(
                    &mut output,
            ...
            AOName::BlockSteal => {
                if self.store[self.store_len - 1].player_id()
                    == self.store[self.store_len - 1].opposing_player_id()
                {
                    ...
                    let victim_id: usize = self.store
                        [self.store_len - self.dist_from_turn[self.store_len - 1]]
                        .opposing_player_id();
                    self.add_moves(
                        &mut output,
            ...
            AOName::CollectiveChallenge => {
                ...
                if self.store[self.store_len - 1].opposing_player_id()
                    == self.store[self.store_len - 1].final_actioner()
                {
                    // Pass
                    // FA2
                    ...
                    } else if self.store_len >= 2
                        && self.store[self.store_len - 2].name() == AOName::Steal
                    {
                        ...
                        && self.store[self.store_len - 2].name() == AOName::Assassinate
                    {
                    ...
                    } else if self.store[self.store_len - self.dist_from_turn[self.store_len - 1]]
                        .name()
                        == AOName::Exchange
                    {
                        // In Cases: A1 => ExchangeDraw
                        let exchanger_id: usize = self.store
                            [self.store_len - self.dist_from_turn[self.store_len - 1]]
                            .player_id();
                        self.add_moves(
                            &mut output,
            ...
            AOName::CollectiveBlock => {
                // Case Checked
                debug_assert!(
                    self.store[self.store_len - 2].name() == AOName::ForeignAid,
                    "Collective Block Should only be after ForeignAid!"
                );
            ...
            AOName::Discard => {
                    ...
                    == AOName::Exchange
                {
                    if self.store[self.store_len - 2].name() == AOName::RevealRedraw {
                        // A3 => Exchange Choice
                        let exchange_id: usize = self.store
                            [self.store_len - self.dist_from_turn[self.store_len - 1]]
                            .player_id();
                        self.add_moves(
                            &mut output,
                        ...
                        // A2 => Next Turn
                        debug_assert!(
                            self.store[self.store_len - 2].opposing_player_id()
                                != self.store[self.store_len - 2].final_actioner(),
                            "Bad AOResult in generate_legal_moves Discard Ambassador"
                        );
                        let next_player_id: usize = self.next_player(self.current_player_turn);
                        if self.latest_coins()[next_player_id] >= 10 {
                            self.add_moves(
                        ...
                        debug_assert!(
                            false,
                            "unintended state reached in generate_legal_moves Discard Ambassador"
                        );
                    }
                } else if self.store[self.store_len - self.dist_from_turn[self.store_len - 1]]
                    .name()
                    ...
                    == AOName::Steal
                {
                    ...
                    {
                        // In Cases: C6 C7 C8 C9 | => BlockSteal
                        let blocker_id: usize = self.store
                            [self.store_len - self.dist_from_turn[self.store_len - 1]]
                            .opposing_player_id();
                        if self.latest_influence()[blocker_id] > 0 {
                            self.add_moves(
                    ...
                    debug_assert!(
                        false,
                        "unintended state reached in generate_legal_moves Discard"
                    );
                }
            }
            AOName::RevealRedraw => {
