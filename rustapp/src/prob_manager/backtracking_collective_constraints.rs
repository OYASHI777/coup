...
impl ActionInfo {
    pub fn public(&self) -> Self {
        match self {
            ...
            Self::Discard { discard } => self.clone(),
            Self::RevealRedraw {
                reveal: revealed, ..
            } => Self::RevealRedraw {
                reveal: *revealed,
...
impl SignificantAction {
    ...
    pub fn set_impossible_constraints(&mut self, impossible_constraints: &[[bool; 5]; 7]) {
        self.meta_data
            .set_impossible_constraints(impossible_constraints);
    }
    pub fn add_inferred_constraints(&mut self, player_id: usize, card: Card) {
        ...
        debug_assert!(
            player_id < 6 && self.meta_data.inferred_constraints[player_id].len() < 3
                || player_id == 6 && self.meta_data.inferred_constraints[player_id].len() < 4,
            "bad push"
        );
    }
...
impl BackTrackCollectiveConstraint {
    ...
    fn calculate_stored_move(&mut self, history_index: usize) {
        ...
        log::trace!(
            "calculate_stored_move history_index: {history_index}, player: {} {:?}",
            self.history[history_index].player(),
            self.history[history_index].action_info()
        );
        ...
        match action_info {
            ActionInfo::Start | ActionInfo::StartInferred => return,
        ...
        log::trace!(
            "calculate_stored_move generate_impossible_constraints history_index: {history_index}"
        );
        ...
        log::info!(
            "recalculated_stored_move end: player: {player_id} {} {:?}",
            history_index,
            self.history[history_index].action_info()
        );
        self.printlog();
    ...
    fn calculate_stored_move_initial(&mut self) {
        ...
        self.history[history_index].meta_data = self.to_meta_data();
        log::info!(
            "recalculated_stored_move_initial: {} {:?}",
            history_index,
            self.history[history_index].action_info()
        );
        self.printlog();
...
impl BackTrackCollectiveConstraint {
    ...
    pub fn player_cards_known(&self, player_id: usize) -> u8 {
        (self.public_constraints[player_id].len() + self.inferred_constraints[player_id].len())
            as u8
    }
    ...
    pub fn add_inferred_player_constraint(&mut self, player_id: usize, card: Card) {
        ...
        debug_assert!(
            self.inferred_constraints[player_id].len() < 2,
            "Adding inferred knowledge to fully known player!"
        );
        // PD Obsolete
    ...
    pub fn reveal_redraw_same(&mut self, player_id: usize, card: Card) {
        if !self.inferred_constraints[player_id].contains(&card) {
            log::trace!(
                "reveal_redraw_same player_id: {}, card: {:?}",
                player_id,
                card
            );
            self.inferred_constraints[player_id].push(card);
    ...
    pub fn reveal_redraw_diff(&mut self, player_id: usize, reveal: Card, redraw: Card) {
        ...
        if let Some(pos) = self.inferred_constraints[6]
            .iter()
            .position(|c| *c == redraw)
        {
    ...
    pub fn update_past_move_hidden_info(&mut self) -> bool {
        ...
        match action_info {
            ActionInfo::RevealRedraw {
                reveal: reveal_considered,
                redraw: redraw_considered,
                relinquish,
            } => {
                match redraw_considered {
                    ...
                    None => {
                        ...
                        for i in (2..index).rev() {
                            ...
                            match action_name {
                                ActionInfoName::Discard => {
                                    if let ActionInfo::Discard { discard } =
                                        action_data.action_info()
                                    {
                                ...
                                ActionInfoName::RevealRedraw => {
                                    log::trace!(
                                        "lookback_initial RevealRedraw checking past RevealRedraw"
                                    );
                                    ...
                                    let need_redraw_update = self.need_redraw_update_2(
                                        i,
                                        reveal_considered,
                                        &illegal_to_change,
                                    );
                                    if let ActionInfo::RevealRedraw {
                                        reveal: reveal_i,
                                        redraw: redraw_i,
                                        ..
                                    } = action_data.action_info()
                                    ...
                                    log::trace!(
                                        "need_redraw_update evaluated to {need_redraw_update}"
                                    );
                                    if need_redraw_update {
                                        ...
                                        if let ActionInfo::RevealRedraw { reveal, redraw, .. } =
                                            self.history[i].action_info_mut()
                                        {
                                            log::trace!(
                                                "lookback_initial setting redraw to: {:?}",
                                                reveal_considered
                                            );
                                        ...
                                        if let ActionInfo::RevealRedraw { redraw, .. } =
                                            self.history[i].action_info()
                                        {
                                            if redraw.is_none() {
                                                need_relinquish_update = self
                                                    .need_relinquish_update(i, reveal_considered);
                                            }
                                        }
                        ...
                        debug_assert!(
                            self.history[0].name() == ActionInfoName::Start,
                            "wrong Significant Action at index 0!"
                        );
                        if !bool_skip_start_update {
                            log::trace!("lookback_initial index: {:?}", index);
                            log::trace!(
                                "lookback_initial processed item: {:?}",
                                self.history.last().unwrap().action_info()
                            );
                            log::trace!("lookback_initial adding inferred_constraint to start: (player: {}, card: {:?})", player_index, reveal_considered);
                            log::trace!(
                                "lookback_initial start before: {:?}",
                                self.history.first().unwrap().meta_data()
                            );
                            self.history[0]
                                .add_inferred_constraints(player_index as usize, reveal_considered);
                            log::trace!(
                                "lookback_initial start after: {:?}",
                                self.history.first().unwrap().meta_data()
                            );
                            bool_changes = true;
                        }
            ...
            ActionInfo::Discard {
                    ...
                    match action_name {
                        ActionInfoName::Discard => {
                            ...
                            if let ActionInfo::Discard { discard } = action_data.action_info() {
                                log::trace!(
                                    "saw action_player: {action_player} discard: {:?} in iter",
                                    discard
                                );
                        ...
                        ActionInfoName::RevealRedraw => {
                            ...
                            log::trace!(
                                "lookback_initial original player: {} index: {} RevealRedraw: {:?}",
                                self.history.len() - 1,
                                self.history.len(),
                                action_info
                            );
                            log::trace!(
                                "lookback_initial checked player: {} index: {} RevealRedraw: {:?}",
                                action_data.player(),
                                i,
                                action_data.action_info()
                            );
                            ...
                            if need_redraw_update {
                                log::trace!(
                                    "lookback_initial original index: {} RevealRedraw: {:?}",
                                    self.history.len() - 1,
                                    action_info
                                );
                                log::trace!(
                                    "lookback_initial considering: index: {} player: {} {:?}",
                                    i,
                                    action_data.player(),
                                    action_data.action_info()
                                );
                                ...
                                {
                                    if redraw.is_none() {
                                        need_relinquish_update =
                                            self.need_relinquish_update(i, discard_considered);
                                    }
                                }
                                log::trace!(
                                    "need_relinquish_update evaluated to {need_relinquish_update}"
                                );
                                log::trace!(
                                    "lookback_initial original index: {} RevealRedraw: {:?}",
                                    self.history.len() - 1,
                ...
                if !bool_skip_start_update {
                    ...
                    self.history[0]
                        .add_inferred_constraints(player_index as usize, discard_considered);
                    log::trace!(
                        "lookback_initial start after: {:?}",
                        self.history.first().unwrap().meta_data()
                    );
    ...
    pub fn lookback_1_continual(&mut self, index: usize) -> bool {
        ...
        match action_info {
            ActionInfo::RevealRedraw {
                ...
                match redraw_considered {
                    Some(redraw_card) => {
                        ...
                        for i in (2..index).rev() {
                            match self.history[i].name() {
                                ActionInfoName::RevealRedraw => {
                                    ...
                                    let bool_change_relinquish =
                                        self.history[i - 1].known_card_count(redraw_card) == 2
                                            && !self.history[i - 1].inferred_constraints()
                                                [self.history[i].player() as usize]
                                                .contains(&redraw_card);
                                    ...
                                    if bool_change_relinquish {
                                        log::trace!(
                                            "lookback_1_continual original RevealRedraw: {:?}",
                                            action_info
                                        );
                                        log::trace!(
                                            "lookback_1_continual considering: player: {} {:?}",
                                            self.history[i].player(),
                                            self.history[i].action_info()
                                        );
                                        if let ActionInfo::RevealRedraw {
                                            reveal, relinquish, ..
                                            ...
                                            if *reveal == redraw_card {
                                                ...
                                                if let Some(relinquish_card) = relinquish {
                                                    ...
                                                    *relinquish_card = *reveal;
                                                    self.regenerate_path();
                                                    log::trace!(
                                                        "End Regenerate Path lookback_continual"
                                                    );
                                                    self.printlog();
                                                    return true;
                                                }
                                ...
                                ActionInfoName::ExchangeDrawChoice => {
                                    if let ActionInfo::ExchangeDrawChoice { draw, relinquish } =
                                        self.history[i].action_info()
                                    {
                                ...
                                ActionInfoName::Start | ActionInfoName::StartInferred => {
                                    debug_assert!(
                                        false,
                                        "Should not have Start when index is not 0"
                                    );
                                }
                        ...
                        // Add to start
                        log::trace!(
                            "lookback1_continual adding to Start player_id: 6, card: {:?}",
                            redraw_card
                        );
                        debug_assert!(
                            self.history[0].name() == ActionInfoName::Start,
            ...
            ActionInfo::Start | ActionInfo::StartInferred => {
    ...
    pub fn need_redraw_update_2(
        &self,
        index: usize,
        card_of_interest: Card,
        illegal_players: &[bool; 6],
    ) -> bool {
        log::trace!("need_redraw_update_2 considered: {index}, card_of_interest: {:?}, illegal_players: {:?}", card_of_interest, illegal_players);
        log::trace!(
            "need_redraw_update_2 considered: {index}, player: {:?} move {:?}",
            self.history[index].player(),
            self.history[index].action_info()
        );
        // TODO: OPTIMIZE
        ...
        for i in (index + 1..self.history.len()).rev() {
            ...
            log::trace!(
                "backward_pass checking i: {i} player_i: {player_i}, move: {:?}",
                self.history[i].action_info()
            );
        ...
        log::trace!(
            "need_redraw_update for player {:?}, move: {:?}",
            self.history[self.history.len() - 1].player(),
            self.history[self.history.len() - 1].action_info()
        );
        log::trace!(
            "!illegal_players[action_player as usize]: {:?}",
            !illegal_players[action_player as usize]
        );
        log::trace!(
            "players_had_revealed_or_discarded: {:?}",
            players_had_revealed_or_discarded
        );
        log::trace!(
            "redraw_i.is_none(): {:?}",
        ...
        let bool_2_cards_outside_player =
            if let ActionInfo::RevealRedraw { reveal, redraw, .. } =
                self.history[index].action_info()
            {
        ...
        log::trace!(
            "bool_2_cards_outside_player returned: {}",
        ...
        {
            log::trace!(
                "need_redraw_update: redraw_i.is_none() {}",
                redraw_i.is_none()
            );
        }
        log::trace!("need_redraw_update: illegal_players {:?}", illegal_players);
        log::trace!(
            "need_redraw_update: !illegal_players[action_player] {}",
            !illegal_players[action_player]
        );
        log::trace!(
            "need_redraw_update: players_had_revealed_or_discarded {:?}",
            players_had_revealed_or_discarded
        );
        log::trace!(
            "need_redraw_update: players_had_revealed_or_discarded[action_player] {}",
            players_had_revealed_or_discarded[action_player]
        );
        log::trace!(
            "need_redraw_update: self.history[index - 1].public_constraints()[action_player] {:?}",
            self.history[index - 1].public_constraints()[action_player]
        );
    ...
    pub fn need_relinquish_update(&self, index: usize, card_of_interest: Card) -> bool {
        ...
        {
            if *reveal_i != card_of_interest && redraw_i.is_none() && action_player == latest_player
            {
                log::trace!(
                    "need_relinquish_update *reveal_i != card_of_interest: {}",
                    *reveal_i != card_of_interest
                );
                log::trace!(
                    "need_relinquish_update redraw_i.is_none(): {}",
                    redraw_i.is_none()
                );
                log::trace!(
                    "need_relinquish_update action_player == latest_player: {}",
                ...
                log::trace!(
                    "need_relinquish_update self.impossible_to_have_cards: {}",
                    output
                );
                return output;
            }
    ...
    pub fn impossible_to_have_cards_general(
        ...
        debug_assert!(
            player_of_interest != 6 && cards.iter().sum::<u8>() <= 2
                || player_of_interest == 6 && cards.iter().sum::<u8>() <= 3,
            "cards too long!"
        );
        let mut public_constraints: Vec<Vec<Card>> = vec![
            Vec::with_capacity(3),
            Vec::with_capacity(3),
            Vec::with_capacity(3),
            Vec::with_capacity(3),
    ...
    /// Does Backtracking to determine if at a particular point that particular player could not have had some set of cards at start of turn
    /// Assuming we won't be using this for ambassador?
    pub fn impossible_to_have_cards(
        &self,
        index: usize,
        player_of_interest: usize,
        cards: &[u8; 5],
        ...
        log::trace!(
            "impossible_to_have_cards index: {}, player_of_interest: {}, cards: {:?}",
            index,
            player_of_interest,
            cards
        );
        ...
        match latest_move.action_info() {
            ...
            ActionInfo::ExchangeDrawChoice { draw, relinquish } => {
                inferred_constraints[latest_move.player() as usize]
                    .extend(relinquish.iter().copied());
                inferred_constraints[6].extend(draw.iter().copied());
            }
            ActionInfo::ExchangeDraw { .. } => todo!(),
            ActionInfo::ExchangeChoice { .. } => todo!(),
    ...
    pub fn possible_to_have_cards_recurse(
        ...
        log::trace!(
            ...
            self.history[index_loop].public_constraints(),
            inferred_constraints
        );
        ...
        if index_loop == index_of_interest - 1 {
            ...
            for (card_num, (query_card_count, current_card_count)) in
                cards.iter().zip(current_card_counts.iter()).enumerate()
            {
            ...
            let fulfilled: bool =
                self.is_valid_combination(index_loop, index_of_interest, inferred_constraints);
            ...
            if !fulfilled {
                    ...
                    cards.iter().zip(current_card_counts.iter()).enumerate()
                {
                    if *query_card_count > *current_card_count {
                        for _ in 0..(*query_card_count - *current_card_count) {
                            if let Some(pos) = inferred_constraints[player_of_interest]
                                .iter()
                                .rposition(|c| *c as usize == card_num)
                            {
        ...
        match self.history[index_loop].action_info() {
            ActionInfo::Discard { discard } => {
                ...
                if let Some(pos) = public_constraints[player_loop]
                    .iter()
                    .rposition(|c| *c == *discard)
                {
                ...
                if let Some(pos) = inferred_constraints[player_loop]
                    .iter()
                    .rposition(|c| *c == *discard)
                {
            ...
            ActionInfo::RevealRedraw {
                ...
                match redraw {
                    Some(redraw_i) => {
                        ...
                        if let Some(pos) =
                            inferred_constraints[6].iter().rposition(|c| *c == *reveal)
                        {
                        ...
                        if let Some(pos) = inferred_constraints[6]
                            .iter()
                            .rposition(|c| *c == *redraw_i)
                        {
                            inferred_constraints[6].swap_remove(pos);
                                    ...
                                    && !inferred_constraints[6].contains(&reveal)
                                {
                                    ...
                                    if index_loop == index_of_interest - 1 {
                                            ...
                                            cards.iter().zip(current_card_counts.iter()).enumerate()
                                        {
                                            if *query_card_count > *current_card_count {
                                                for _ in
                                                    0..(*query_card_count - *current_card_count)
                                                {
                                                    if let Some(pos) = inferred_constraints
                                                        [player_of_interest]
                                                        .iter()
                                                        .rposition(|c| *c as usize == card_num)
                                                    {
                                ...
                                if inferred_constraints[player_loop].is_empty() {
                                    ...
                                    if inferred_constraints[player_loop].len() < 3
                                        && inferred_constraints
                                            .iter()
                                            .map(|v| {
                                                v.iter().filter(|c| **c == *reveal).count() as u8
                                            })
                                            .sum::<u8>()
                                            < 4
                                    {
                                    ...
                                    if inferred_constraints[player_loop].len() < 2 {
                                        ...
                                        if response {
                                            if index_loop == index_of_interest - 1 {
                                                for (
                                                    card_num,
                                                    (query_card_count, current_card_count),
                                                ) in cards
                                                    .iter()
                                                    .zip(current_card_counts.iter())
                                                    .enumerate()
                                                {
                                                    if *query_card_count > *current_card_count {
                                                        for _ in 0..(*query_card_count
                                                            - *current_card_count)
                                                        {
                                                            if let Some(pos) = inferred_constraints
                                                                [player_of_interest]
            ...
            ActionInfo::ExchangeDrawChoice { .. } => {
                response = self.recurse_variants_exchange(
                    index_loop,
                    index_of_interest,
                    player_of_interest,
                    public_constraints,
    ...
    pub fn is_valid_combination(
        ...
        for player in 0..7 {
            if inferred_constraints[player].len() == 1
                && self.history[index_loop].impossible_constraints()[player]
                    [inferred_constraints[player][0] as usize]
            {
                return false;
            }
            if inferred_constraints[player].len() == 2
                && self.history[index_loop].impossible_constraints_2()[player]
                    [inferred_constraints[player][0] as usize]
        ...
        if inferred_constraints[6].len() == 3
            && self.history[index_loop].impossible_constraints_3()
                [inferred_constraints[6][0] as usize][inferred_constraints[6][1] as usize]
                [inferred_constraints[6][2] as usize]
        {
            return false;
        }
        // =========================================
        for player in 0..7 {
            ...
            inferred_constraints[player]
                .iter()
                .for_each(|c| current_card_counts[*c as usize] += 1);

            let mut required_card_counts: [u8; 5] = [0; 5];
            self.history[index_loop].inferred_constraints()[player]
                .iter()
                .for_each(|c| required_card_counts[*c as usize] += 1);
            self.history[index_loop].public_constraints()[player]
                .iter()
                .for_each(|c| required_card_counts[*c as usize] += 1);
            ...
            current_card_counts
                .iter()
                .zip(required_card_counts.iter())
                .for_each(|(cur, req)| total_count += *cur.max(req));
            ...
            if !fulfilled {
                log::trace!(
                    "is_valid_combination player {} failed to fulfil previous state!",
                    player
                );
                return false;
            }
        ...
        for player in 0..6 as usize {
            match public_constraints[player].len() + inferred_constraints[player].len() {
                ...
                1 => {
                    if action_public_constraint[player].len()
                        + action_inferred_constraint[player].len()
                        == 2
                    {
        ...
        if inferred_constraints[6].len() + action_inferred_constraint[6].len() > 3 {
            ...
            action_inferred_constraint[6]
                .iter()
                .for_each(|c| action_inferred_counts[*c as usize] += 1);
            inferred_constraints[6]
                .iter()
                .for_each(|c| inferred_counts[*c as usize] += 1);
            action_inferred_counts
                .iter()
                .zip(inferred_counts.iter())
                .for_each(|(a, i)| {
                    if *i > *a {
                        count_total += *i.max(a);
                    }
    ...
    pub fn recurse_variants_exchange(
        &self,
        index_loop: usize,
        index_of_interest: usize,
        player_of_interest: usize,
        ...
        let player_lives =
            2 - self.history[index_loop].public_constraints()[player_loop].len() as u8;
            ...
            if inferred_constraints[player_loop].len() == 0 && inferred_constraints[6].len() > 1 {
                for index_pile_to_player_0 in 0..iter_cards_pile.len() {
                    for index_pile_to_player_1 in index_pile_to_player_0..iter_cards_pile.len() {
                        ...
                        inferred_constraints[player_loop]
                            .push(iter_cards_pile[index_pile_to_player_0]);
                        inferred_constraints[player_loop]
                            .push(iter_cards_pile[index_pile_to_player_1]);
                        // let mut temp = inferred_constraints.clone();
                        // temp[player_loop] = player_hand.clone();
                        // temp[6] = pile_hand.clone();
                        // variants.push(temp);
                ...
                for card_pile in iter_cards_pile.iter() {
                    for index_player_to_pile_0 in 0..iter_cards_player.len() {
                            ...
                            index_player_to_pile_0..iter_cards_player.len()
                        {
                            ...
                            // let mut pile_hand = inferred_constraints[6].clone();
                            let (
                                mut bool_player_removed_0,
                                mut bool_player_removed_1,
                                mut bool_pile_removed_0,
                            ) = (false, false, false);
                            if let Some(pos) = inferred_constraints[player_loop]
                                .iter()
                            ...
                            if bool_player_removed_1 {
                                inferred_constraints[player_loop]
                                    .push(iter_cards_player[index_player_to_pile_1]);
                            }
                            if bool_player_removed_0 {
                                inferred_constraints[player_loop]
                                    .push(iter_cards_player[index_player_to_pile_0]);
                            }
                        }
                    }
                }
    ...
    fn generate_impossible_constraints(&mut self, history_index: usize) {
        ...
        for player_of_interest in 0..7 {
            ...
            for card in 0..5 {
                ...
                self.impossible_constraints[player_of_interest][card] = self
                    .impossible_to_have_cards_general(history_index, player_of_interest, &cards);
                cards[card] = 0;
            }
        }
        for player_of_interest in 0..7 {
    ...
    fn generate_inferred_constraints(&mut self) {
        ...
        for player in 0..6 {
            if self.public_constraints[player].len() == 0 {
                if self.impossible_constraints[player]
                    .iter()
                    .map(|b| !*b as u8)
                    .sum::<u8>()
                    == 1
                {
                    if let Some(card_num) =
...
impl CoupConstraint for BackTrackCollectiveConstraint {
    ...
    fn add_move(&mut self, player_id: u8, action: ActionInfo) {
        match action {
            ActionInfo::Discard { .. } => {
                let significant_action =
                    SignificantAction::initial(self.move_no, player_id, action);
